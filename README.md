...
- **프로필별 분리** — 한 기기에서 가족·친구가 각자 이름으로 들어와도 데이터가 안 섞임. 통계 화면은 전체/개인 탭으로 분리.
- **오늘 평가서 이미지** — 기록 화면 상단에서 오늘 운동·식단·신체 요약을 한 장의 PNG 카드로 저장 (공유용).
- **신체 정보 캐리오버** — 새 프로필 시작 시 신체 정보 입력, 이후엔 매일 안 재도 마지막 체중이 오늘 자동 이어짐.
...
| `/#/settings` | 설정 — 프로필 전환·삭제, JSONL 내보내기/가져오기(병합/덮어쓰기), 내가 등록한 음식 관리, 위험 구역. **관리자 전용 화면** (지정 프로필이거나 관리자 비밀번호 통과 시 접근). |
